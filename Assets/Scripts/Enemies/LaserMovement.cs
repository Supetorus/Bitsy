...
public class LaserMovement : MonoBehaviour
{
  ...
  void Update() {
    if (isVerticalSliding) {
      ...
      if (slidingUp) {
        transform.position = Vector3.MoveTowards(transform.position, new Vector3(startingTransform.x, startingTransform.y + movementAmount, startingTransform.z), .1f * Time.deltaTime);
      } else if (slidingDown) {
        transform.position = Vector3.MoveTowards(transform.position, new Vector3(startingTransform.x, startingTransform.y - movementAmount, startingTransform.z), .1f * Time.deltaTime);
      }
      ...
      if (slidingLeftX) {
        transform.position = Vector3.MoveTowards(transform.position, new Vector3(startingTransform.x + movementAmount, startingTransform.y, startingTransform.z), .1f * Time.deltaTime);
      ...
      //move the laser left and right
      if (slidingLeftZ) {
        transform.position = Vector3.MoveTowards(transform.position, new Vector3(startingTransform.x, startingTransform.y, startingTransform.z + movementAmount), .1f * Time.deltaTime);
